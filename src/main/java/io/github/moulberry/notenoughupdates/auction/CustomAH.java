...
public class CustomAH extends Gui {
	...
	private static final ItemStack CONTROL_LEVEL = Utils.createItemStack(Items.ghast_tear, EnumChatFormatting.GREEN + "Pet Level");
	...
	private final ItemStack[] controls = {DUNGEON_SORT, CONTROL_SORT, CONTROL_TIER, CONTROL_LEVEL, CONTROL_MYAUC, null, CONTROL_BIN, CONTROL_ENCH, CONTROL_STATS};
	...
	public List<String> getTooltipForControl(int index) {
		...
		switch (index) {
			...
			case 3:
				...
				lore.add(selPrefix + "Minimum Pet Level: " + EnumChatFormatting.YELLOW + lvl);
				return lore;
			case 4:
				lore.add("");
	...
	private boolean doesAucMatch(APIManager.Auction auc) {
		...
		if (levelFilter > PET_LEVEL_0) {
			switch (levelFilter) {
				case PET_LEVEL_80:
					match &= auc.petLevel >= 80;
					break;
				case PET_LEVEL_90:
					match &= auc.petLevel >= 90;
					break;
				case PET_LEVEL_100:
					match &= auc.petLevel >= 100;
					break;
			}
		}
